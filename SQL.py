...
#This function has the capability to write the outputs of acategory computation to a SQLite database,
#taking into account the specific requirements mentioned in the assignment.

# Independently compile a SQLite database (file) ideally via sqlalchemy and load the training data into 
#a single fivecolumn spreadsheet / table in the file

def to_sqlite_deviate_data(output):
  
    copy_to_sqlite= create_engine('sqlite:///{}.db'.format("data_structure"), echo=False)
    metadata = MetaData(copy_to_sqlite)

    data_structure = Table('data_structure', metadata,
                    Column('X (test function)', Float, primary_key=False),
                    Column('Y (test function)', Float),
                    Column('Delta Y (test function)', Float),
                    Column('No. of ideal function', String(50))
                    )
    ...
    for i in output:
        ...
        if category is not None:
            category_name =category.name.replace("y", "N")
        else:
            # If there is nocategory, there is also no distance. In that case I write a dash
            category_name = "-"
            Delta_Y = -1

        map_run.append(
            {"X (test function)": pnt["x"], "Y (test func)": pnt["y"], "diff Y (test func)": Delta_Y,
             "No. of ideal func":category_name})

    # using the Table object, the dict is used to insert the data
    i = data_structure.insert()
    i.execute(map_run)
